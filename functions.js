...
function showGames()
{
  ...
  .then(function (gameInformation){
    ...
    gameInformation.forEach(function(currentGame)
    {
        tableCode += 
        `<tr>\
            <td><a class="blueText" onclick="goToLeaderboard(${currentGame.id})">${currentGame.GameName}</a></td>\
...
//  $ end if the string
//  . means any character
//  quantifiers (takes the character to the left and looks for any number based quantifier)
  //  ? 0 - 1 instance
  //  + 1 - many instances
  //  * 0 - many instances
  //  \s <- white space
...
//get games from https://lime-faithful-hippo.cyclic.app/api/games
//get games from https://lime-faithful-hippo.cyclic.app/api/leaderboard/${id}
